...
5. Keep audit events difficult for the ordinary app role to erase and copy them off the VPS with protected backups or an independent log destination. A database administrator or agent with root-equivalent access could still disable local logging; describe that limitation honestly. Restrict access to any before/after values and never log passwords, tokens or unnecessary personal data.
6. Verify with a small real test: create, edit, delete and restore a test record; identify its actor, count and request ID; exercise a bulk operation and check its alert. If recovery needs more precision than snapshots allow, evaluate PostgreSQL base backups plus archived WAL for point-in-time recovery. An SQL dump alone cannot provide WAL-based point-in-time recovery.

Example: if an agent deletes 30 contacts, the record history shows the actor, sponsoring human, affected IDs, count and time; an alert draws attention to the bulk action; the contacts can be restored from the reversible-delete area or a tested backup. A summary in Obsidian is optional and is not the audit source.
...
References: [OWASP logging guidance](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html), [PostgreSQL backup and PITR](https://www.postgresql.org/docs/current/continuous-archiving.html), [pgAudit](https://github.com/pgaudit/pgaudit).
