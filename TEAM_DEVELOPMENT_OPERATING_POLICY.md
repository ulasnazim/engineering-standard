...
**Version:** 2.0 (owner-approved human discretion and recovery defaults)
**Date:** 2026-09-23  
...
**The one-server trade-off:** If the VPS fails, *every hosted application can be unavailable at once*. Set per-project recovery targets (maximum data loss and acceptable downtime). Automate **one dated, encrypted archive file per backup run** with database-consistent exports, application files/uploads, necessary configuration and a restore manifest; upload it to **Ulaş's Google Drive**. Retain multiple dated versions (initial target: 7 daily, 4 weekly, 6 monthly). Automate freshness, upload, integrity and Drive-quota checks with failure alerts; regularly download and decrypt a copy. Test an isolated restore at least monthly, automated where practical; an intact archive alone does not prove the application can be recovered. Keep the decryption key recoverable by Ulaş independently of the VPS and Drive, and back up before high-risk migrations. If a product cannot afford the data loss between snapshots, evaluate PostgreSQL point-in-time recovery as an additional, tested mechanism. Never restore test data over production. GitHub source does not replace production backups. The implementer follows [the VPS backup brief](docs/owner/VPS_BACKUP_AUTOMATION.md); publishing this policy does not install the backup job on the VPS.

**Customer and business records:** Each product holding valuable records should automatically keep a recoverable change history: reversible deletion when retention permits, actor and sponsoring human for agent actions, timestamp, affected IDs/count, and alerts for unexpected bulk deletion. Account for database changes outside the application where relevant, and keep a protected offsite copy of audit history. Do not require developers to type each action into a separate journal. Obsidian may contain readable summaries or incident notes; it is not the primary audit log or recovery system. See [data-change guidance](docs/DATA_CHANGE_RECOVERY.md). These are defaults a responsible human may adapt with a short note for material consequences.
